...
df_tiendas = al.agregar_indicadores_apertura(df_tiendas)
if "SCOREF" not in df_tiendas.columns:
    df_tiendas["SCOREF"] = pd.NA
df_tiendas["clasificacion_scoref"] = df_tiendas.apply(
    lambda row: al.clasificar_scoref(row["SCOREF"], row.get("es_tienda_nueva", False), row.get("TIE27")), axis=1
)
...
segmentos = ["Todos"] + [x for x in ["Mejor tienda", "Tienda buena", "En observación", "En alerta", "EXP 2025 / expansión", "EXP 2026 / expansión", "Sin SCOREF"] if x in df_t["clasificacion_scoref"].unique()]
...
with tab_resumen:
    ...
    c4.metric("SCOREF promedio TMCB", f"{df_t.loc[df_t['TIE27'].eq('TMCB'), 'SCOREF'].mean():.2f}" if df_t.loc[df_t['TIE27'].eq('TMCB'), 'SCOREF'].notna().any() else "Sin dato")
    c5.metric("Tiendas en alerta", f"{len(al.alertas_scoref(df_t)):,}")

    st.markdown(
        '<div class="legend"><b>Lectura rápida:</b> SCOREF mayor a 3 representa el mejor desempeño; entre 0 y 3 son tiendas buenas; de -3 a menos de 0 requieren observación; menor a -3 son tiendas en alerta. La alerta de SCOREF se evalúa para <b>TMCB</b>; <b>EXP 2025</b> y <b>EXP 2026</b> se muestran como expansión.</div>',
        unsafe_allow_html=True,
    )
    c1, c2 = st.columns(2)
    with c1:
    ...
    criticas = al.alertas_scoref(df_t)
    cols = ["CR", "NAME", "PLAZA 2026", "TIE27", "Mesop", "FECHA APE", "VENTAS OUM", "SCOREF", "clasificacion_scoref"]
...
with tab_tiendas:
    ...
    cols_show = ["CR", "NAME", "PLAZA 2026", "MUNICIPIO", "ESTADO", "TIE27", "Mesop", "FECHA APE", "SCOREF", "clasificacion_scoref", "GENERADOR", "TRAFICO UM", "TICKET UM", "VENTAS OUM", "CONTRIBUCION UM", "MARGEN UM", "MARGEN_%_UM", "RENTA UM", "%Batting"]
